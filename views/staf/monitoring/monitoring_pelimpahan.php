...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $id_pelimpahan = $row['id_pelimpahan'];

        if ($kategori === 'Meninggal Dunia') {
            $detail_query = "SELECT * FROM pelimpahan_meninggal WHERE id_pelimpahan = ?";
        } else {
            $detail_query = "SELECT * FROM pelimpahan_sakit WHERE id_pelimpahan = ?";
        }

        $stmt = $koneksi->prepare($detail_query);
        $stmt->bind_param("i", $id_pelimpahan);
        $stmt->execute();
...
$doc_fields = [
    ...
    'Sakit Permanen' => [
        'dokumen_surat_sakit' => ['label' => 'Surat Sakit', 'status_field' => 'dokumen_surat_sakit_status'],
        'dokumen_setor_awal' => ['label' => 'Setor Awal', 'status_field' => 'dokumen_setor_awal_status'],
        'dokumen_spph' => ['label' => 'SPPH', 'status_field' => 'dokumen_spph_status'],
        'dokumen_ahli_waris' => ['label' => 'Ahli Waris', 'status_field' => 'dokumen_ahli_waris_status'],
        'dokumen_surat_kuasa' => ['label' => 'Surat Kuasa', 'status_field' => 'dokumen_surat_kuasa_status'],
...
function updateValidationDateIfAllVerified($id_pelimpahan, $koneksi, $doc_fields)
{
    // Ambil data pelimpahan
    $query = "SELECT kategori FROM pelimpahan WHERE id_pelimpahan = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        // Ambil data dokumen
        $table = ($kategori === 'Meninggal Dunia') ? 'pelimpahan_meninggal' : 'pelimpahan_sakit';
        $doc_query = "SELECT * FROM $table WHERE id_pelimpahan = ?";
        $doc_stmt = $koneksi->prepare($doc_query);
        $doc_stmt->bind_param("i", $id_pelimpahan);
        $doc_stmt->execute();
        ...
        if ($doc_result->num_rows > 0) {
            ...
            if (isAllDocumentsVerified($combined_data, $doc_fields)) {
                ...
                $check_query = "SELECT tanggal_validasi FROM pelimpahan WHERE id_pelimpahan = ?";
                $check_stmt = $koneksi->prepare($check_query);
                $check_stmt->bind_param("i", $id_pelimpahan);
                $check_stmt->execute();
                ...
                if (empty($check_data['tanggal_validasi'])) {
                    ...
                    $update_query = "UPDATE pelimpahan SET tanggal_validasi = ? WHERE id_pelimpahan = ?";
                    $update_stmt = $koneksi->prepare($update_query);
                    $update_stmt->bind_param("si", $tanggal_validasi, $id_pelimpahan);
                    $update_stmt->execute();
                    $update_stmt->close();
                }
                $check_stmt->close();
            } else {
                $reset_query = "UPDATE pelimpahan SET tanggal_validasi = NULL WHERE id_pelimpahan = ?";
                $reset_stmt = $koneksi->prepare($reset_query);
                $reset_stmt->bind_param("i", $id_pelimpahan);
                $reset_stmt->execute();
                $reset_stmt->close();
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['validate_document'])) {
    ...
    $id_pelimpahan = clean_input($_POST['id_pelimpahan']);
    ...
    $query_kategori = "SELECT kategori FROM pelimpahan WHERE id_pelimpahan = ?";
    $stmt_kategori = $koneksi->prepare($query_kategori);
    $stmt_kategori->bind_param("i", $id_pelimpahan);
    ...
    if ($result_kategori->num_rows > 0) {
        ...
        if (isset($doc_fields[$kategori][$document_field])) {
            ...
            $table = ($kategori === 'Meninggal Dunia') ? 'pelimpahan_meninggal' : 'pelimpahan_sakit';
            $query = "UPDATE $table SET $status_field = ? WHERE id_pelimpahan = ?";

            $stmt = $koneksi->prepare($query);
            if ($stmt) {
                $stmt->bind_param("si", $status_value, $id_pelimpahan);

                if ($stmt->execute()) {
                    updateAktivitasPengguna($id_staf, 'staf', 'Pelimpahan', 'Memperbarui status pelimpahan');
                    // Update tanggal validasi jika semua dokumen terverifikasi
                    updateValidationDateIfAllVerified($id_pelimpahan, $koneksi, $doc_fields);
        ...
        echo "<script>alert('Error: Data pelimpahan tidak ditemukan'); window.location.href = '" . $_SERVER['PHP_SELF'] . "';</script>";
        exit;
    }
    $stmt_kategori->close();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_pelimpahan'], $_POST['action']) && !isset($_POST['validate_document'])) {
    $id_pelimpahan = clean_input($_POST['id_pelimpahan']);
    ...
    $query = "UPDATE pelimpahan SET status = ?, tanggal_validasi = ? WHERE id_pelimpahan = ?";
    $stmt = $koneksi->prepare($query);

    if ($stmt) {
        $stmt->bind_param("ssi", $status, $tanggal_validasi, $id_pelimpahan);

        if ($stmt->execute()) {
            updateAktivitasPengguna($id_staf, 'staf', 'Pelimpahan', 'Memperbarui status pelimpahan');
            $_SESSION['success_message'] = "Status pelimpahan berhasil diperbarui";
            echo "<script>alert('Status pelimpahan berhasil diperbarui!'); window.location.href = '" . $_SERVER['PHP_SELF'] . "';</script>";
            exit;
        } else {
    ...
    <div class="layout-content">
        ...
        <main class="monitoring-wrapper">
            <div class="monitoring">
                    <div class="monitoring-header" style="color: white;">
                        <i class="fas fa-table me-1"></i> Berkas Menunggu Verifikasi Pelimpahan Jamaah Haji
                    </div>
                    <div class="monitoring-body">
                        ...
                        <div class="mb-3">
                            <small class="text-muted">
                                ...
                                <i class="fas fa-square text-warning"></i> Sakit Permanen
                            </small>
                        </div>

                        <table id="tabelPelimpahan" class="table table-striped table-bordered table-hover">
                                ...
                                if (!empty($data_pembatalan)) {
                                    foreach ($data_pembatalan as $row) {
                                        ...
                                        if ($row['kategori'] === 'Meninggal Dunia') {
                                            $row_class = 'kategori-merah';
                                        } elseif ($row['kategori'] === 'Sakit Permanen') {
                                            $row_class = 'kategori-kuning';
                                        }
                                        ...
                                        $badge_class = ($row['kategori'] === 'Meninggal Dunia') ? 'badge-kategori-merah' : 'badge-kategori-kuning';
                                        ...
                                        foreach ($documents as $field => $info) {
                                            ...
                                            $modal_id = "modalCatatan_" . $row['id_pelimpahan'] . "_" . $field_safe;
                                            ...
                                            if (!empty($file)) {
                                                ...
                                                if ($doc_status == 'Terverifikasi') {
                                                    ...
                                                    echo "<input type='hidden' name='id_pelimpahan' value='" . $row['id_pelimpahan'] . "'>";
                                                    echo "<input type='hidden' name='document_field' value='$field'>";
                                                    echo "<input type='hidden' name='validation_status' value='Terverifikasi'>";
                                                    echo "<button type='submit' class='btn btn-outline-success' title='Terverifikasi'>";
                                                    ...
                                                    echo "<input type='hidden' name='document_field' value='$field'>";
                                                    echo "<input type='hidden' name='validation_status' value='Terverifikasi'>";
                                                    echo "<button type='submit' class='btn btn-success' title='Terverifikasi'>";
                                                    echo "<i class='fas fa-check'></i></button>";
                                                    echo "</form>";
                                                ...
                                                echo "<button class='btn btn-secondary' disabled style='min-width: 120px;'><i class='fas fa-file-slash'></i> $label</button>";
                                    ...
                                    echo "<tr><td colspan='10' class='text-center'>Tidak ada data berkas pelimpahan jamaah haji yang ditemukan.</td></tr>";
                                }
                                ?>
                            </tbody>
                        </table>
            ...
            <div class='modal-content'>
                ...
                <form method='post'>
                    <div class='modal-body'>
                        ...
                        <input type='hidden' name='id_pelimpahan' value='{$modal['id_pelimpahan']}'>
