...
function uploadFile($input_name, $folder_jamaah, $prefix, $old_path = null)
{
    if (isset($_FILES[$input_name]) && $_FILES[$input_name]['error'] === UPLOAD_ERR_OK) {
        ...
        // Path fisik: naik 5 tingkat ke root/uploads
        ...
        if (move_uploaded_file($_FILES[$input_name]['tmp_name'], $physical_path)) {
            // Hapus file lama jika user upload yang baru
            if ($old_path) {
    ...
    // Jika tidak ada upload baru, kembalikan path yang lama
    return $old_path;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Nama folder tetap konsisten
    $safe_nama = preg_replace('/[^A-Za-z0-9\-]/', '_', $nama_jamaah);
    $folder_jamaah = $safe_nama . "-" . time();

    // Upload dokumen baru & hapus yang lama jika ada
    $dokumen_akta_kematian      = uploadFile('dokumen_akta_kematian', $folder_jamaah, 'AktaKematian', $data['dokumen_akta_kematian']);
    ...
    $dokumen_ahli_waris         = uploadFile('dokumen_ahli_waris', $folder_jamaah, 'KeteranganAhliWaris', $data['dokumen_ahli_waris']);
    $dokumen_surat_kuasa        = uploadFile('dokumen_surat_kuasa', $folder_jamaah, 'SuratKuasa', $data['dokumen_surat_kuasa']);
    $dokumen_ktp_ahliwaris      = uploadFile('dokumen_ktp_ahliwaris', $folder_jamaah, 'KTP_AhliWaris', $data['dokumen_ktp_ahliwaris']);
    $dokumen_ktp_penerima_kuasa = uploadFile('dokumen_ktp_penerima_kuasa', $folder_jamaah, 'KTP_PenerimaKuasa', $data['dokumen_ktp_penerima_kuasa']);
    $dokumen_kk_penerima_kuasa  = uploadFile('dokumen_kk_penerima_kuasa', $folder_jamaah, 'KK_PenerimaKuasa', $data['dokumen_kk_penerima_kuasa']);
    $dokumen_akta_kelahiran     = uploadFile('dokumen_akta_kelahiran', $folder_jamaah, 'AktaLahir_AhliWaris', $data['dokumen_akta_kelahiran']);
    $dokumen_buku_nikah         = uploadFile('dokumen_buku_nikah', $folder_jamaah, 'BukuNikah', $data['dokumen_buku_nikah']);
    $dokumen_rekening_kuasa     = uploadFile('dokumen_rekening_kuasa', $folder_jamaah, 'RekeningKuasa', $data['dokumen_rekening_kuasa']);
    $foto_wajah                 = uploadFile('foto_wajah', $folder_jamaah, 'Foto_PenerimaKuasa', $data['foto_wajah']);
