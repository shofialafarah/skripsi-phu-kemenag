...
if (isset($_POST['verifikasi'])) {
    $idPembatalan = $_POST['id_pembatalan'];
    $status = $_POST['validation_status'];
    $tanggalVerifikasi = date('Y-m-d H:i:s');

    $updateQuery = "UPDATE pembatalan 
                    SET status_verifikasi = '$status', 
                        tanggal_verifikasi = '$tanggalVerifikasi' 
                    WHERE id_pembatalan = $idPembatalan";

    if (mysqli_query($koneksi, $updateQuery)) {
        echo "<script>alert('Verifikasi berhasil disimpan!'); window.location='verifikasi_pembatalan.php';</script>";
    } else {
...
if ($filterKategori === 'Keperluan Ekonomi' || $filterKategori === 'semua') {
    $query .= "
    SELECT 
        'Ekonomi' AS jenis,
        p.id_pembatalan,
        e.id_batal_ekonomi AS id_cetak,
        e.nama_jamaah,
        e.nomor_porsi,
        e.alamat,
        e.no_telepon,
        ...
        p.tanggal_verifikasi,
         p.tanggal_validasi
    FROM pembatalan_ekonomi e
    JOIN pembatalan p ON e.id_pembatalan = p.id_pembatalan
    ";
    if ($filterKategori === 'Keperluan Ekonomi') {
        $query .= "WHERE p.kategori = 'Keperluan Ekonomi' ";
    }
}
...
if ($filterKategori === 'Meninggal Dunia' || $filterKategori === 'semua') {
    ...
    SELECT 
        'Meninggal' AS jenis,
        p.id_pembatalan,
        m.id_batal_meninggal AS id_cetak,
        m.nama_jamaah,
        m.nomor_porsi,
        m.alamat,
        ...
        p.tanggal_verifikasi,
         p.tanggal_validasi
    FROM pembatalan_meninggal m
    JOIN pembatalan p ON m.id_pembatalan = p.id_pembatalan
    ";
...
$query = "SELECT * FROM ( $query ) AS gabungan ORDER BY tanggal_pengajuan DESC";
...
<div class="layout">
    ...
    <div class="layout-content">
        <?php include_once __DIR__ . '/../includes/header_kasi.php'; ?>
            ...
            <main class="pembatalan-wrapper">
                <div class="pembatalan">
                    <div class="pembatalan-header" style="background-color: #1b5e20; color: white;">
                        <i class="fas fa-table me-1"></i> Verifikasi Pembatalan Jamaah Haji
                    </div>
                    <div class="pembatalan-body" style="color: #1b5e20;">
                                ...
                                <div class="col-md-6">
                                    <form method="GET" class="filter-form d-flex align-items-center gap-2">
                                        ...
                                        <select id="filter_kategori" name="filter_kategori" class="form-select" style="width: auto;">
                                            ...
                                            <option value="Keperluan Ekonomi" <?= $filterKategori == 'Keperluan Ekonomi' ? 'selected' : '' ?>>Ekonomi</option>
                        ...
                        <?php if ($result && mysqli_num_rows($result) > 0): ?>
                            ...
                            <div class="table-responsive">
                                <table id="tabelPembatalan" class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            $badgeClass = ($kategori === 'Meninggal Dunia') ? 'badge bg-danger text-white' : (($kategori === 'Keperluan Ekonomi') ? 'badge bg-warning text-dark' : '');
                                            ...
                                            $linkCetak = ($row['kategori'] === 'Meninggal Dunia')
                                                ? "cetak_pembatalan_meninggal.php?id=" . $row['id_cetak']
                                                : "cetak_pembatalan_ekonomi.php?id=" . $row['id_cetak'];
        ...
        onclick='openVerificationModal(" . $row['id_pembatalan'] . ")'>
        <i class='fas fa-edit'></i> Opsi
                        ...
                        <?php else: ?>
                            <div class="alert alert-info text-center">
                                <i class="fas fa-info-circle"></i> Tidak ada data pembatalan yang ditemukan.
                            </div>
                        <?php endif; ?>
                    </div>
        ...
        <div class="modal-dialog">
            <form method="POST" class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="verificationModalLabel">Verifikasi Pembatalan</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="verifikasi" value="1">
                    <input type="hidden" name="id_pembatalan" id="modal_id_pendaftaran">
...
<script src="../assets/js/sidebar.js"></script>
    ...
    <script src="tanggal_verifikasi_pembatalan.js"></script>
    <script>
