...
function hapusFoto() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      // Ambil nama file saat ini (edit_kasi.php atau tambah_kasi.php)
      const currentPage = window.location.pathname.split("/").pop();

      if (id && currentPage === "edit_kasi.php") {
        // Jika di halaman EDIT: Kirim perintah hapus ke database
        window.location.href = `edit_kasi.php?id=${id}&remove_foto=true`;
      } else {
        ...
        if (preview)
          preview.src =
            "/phu-kemenag-banjar-copy/views/admin/akun-pengguna/kasi/assets/img/profil.jpg";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (params.get("deleted") === "1") {
    // Use the same success message as the update flow
    Swal.fire("Berhasil", "Data kasi berhasil diperbarui.", "success");
  ...
  if (params.get("created") === "1") {
    ...
    Swal.fire("Gagal", "Gagal menambahkan data kasi.", "error");
  }

  if (params.get("updated") === "1") {
    Swal.fire("Berhasil", "Data kasi berhasil diperbarui.", "success");
  } else if (params.get("updated") === "0") {
    Swal.fire("Gagal", "Gagal memperbarui data kasi.", "error");
  }
