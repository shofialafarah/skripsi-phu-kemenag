...
function hapusFoto() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      // Ambil nama file saat ini (edit_jamaah.php atau tambah_jamaah.php)
      const currentPage = window.location.pathname.split("/").pop();

      if (id && currentPage === "edit_jamaah.php") {
        // Jika di halaman EDIT: Kirim perintah hapus ke database
        window.location.href = `edit_jamaah.php?id=${id}&remove_foto=true`;
      } else {
        ...
        if (preview)
          preview.src =
            "/phu-kemenag-banjar-copy/views/admin/akun-pengguna/jamaah/assets/img/profil.jpg";
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (params.get('created') === '1') {
        if (params.get('mail') === '0') {
            Swal.fire('Berhasil','Akun berhasil dibuat, tetapi email gagal dikirim.','warning');
        } else {
            Swal.fire('Berhasil','Akun berhasil dibuat dan email dikirim.','success');
        }
    } else if (params.get('created') === '0') {
        Swal.fire('Gagal','Gagal menambahkan data jamaah.','error');
    }

    if (params.get('updated') === '1') {
        Swal.fire('Berhasil','Data jamaah berhasil diperbarui.','success');
    } else if (params.get('updated') === '0') {
        Swal.fire('Gagal','Gagal memperbarui data jamaah.','error');
    }

    // remove known query params without reload
    if (history.replaceState) {
        var url = window.location.href.replace(/\?(deleted|created|mail|updated)=[^&]*(&)?/g, '');
        url = url.replace(/&$/, '');
        history.replaceState(null, '', url);
    }
});