...
$id_staf = $_GET['id'];
$stmt = $koneksi->prepare("SELECT * FROM staf WHERE id_staf = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $foto = $data['foto']; // Default gunakan foto lama
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        // Update data
        $stmt = $koneksi->prepare("UPDATE staf SET 
                  nama_staf = ?, nip = ?, pangkat = ?, posisi = ?, 
                  pend_terakhir = ?, tempat_lahir = ?, tgl_lahir = ?, alamat = ?, email = ?, 
                  username = ?, no_telepon = ?, foto = ? 
                  WHERE id_staf = ?");
...
if (isset($_GET['remove_foto']) && $_GET['remove_foto'] == 'true') {
    ...
    $stmt = $koneksi->prepare("UPDATE staf SET foto = '' WHERE id_staf = ?");
    $stmt->bind_param("i", $id_staf);
    $stmt->execute();
                                ...
                                <div style="display: flex; gap: 10px; align-items: center;">
                                    ...
                                    <div style="flex: 1;">
                                        ...
                                        <select id="posisi" name="posisi" required>
                                            ...
                                            <option value="Staf Administrasi PHU" <?= $data['posisi'] == 'Staf Administrasi PHU' ? 'selected' : ''; ?>>Staf Administrasi PHU</option>
                                            <option value="Staf Verifikasi Dokumen" <?= $data['posisi'] == 'Staf Verifikasi Dokumen' ? 'selected' : ''; ?>>Staf Verifikasi Dokumen dan Visa</option>
                                            <option value="Staf Pemberkasan" <?= $data['posisi'] == 'Staf Pemberkasan' ? 'selected' : ''; ?>>Staf Pemberkasan</option>
                                            <option value="Staf Pelayanan Haji" <?= $data['posisi'] == 'Staf Pelayanan Haji' ? 'selected' : ''; ?>>Staf Pelayanan Haji</option>
                                            <option value="Staf Akomodasi" <?= $data['posisi'] == 'Staf Akomodasi' ? 'selected' : ''; ?>>Staf Akomodasi</option>
                                            <option value="Staf Pendukung" <?= $data['posisi'] == 'Staf Pendukung' ? 'selected' : ''; ?>>Staf Pendukung</option>
                                        </select>
                                    </div>
                                    <div style="flex: 1;">
...
<script>
    ...
    function setDefaultFoto() {
        document.getElementById('previewFoto').src = 'uploads/staf/profil.jpg';
    }
</script>
