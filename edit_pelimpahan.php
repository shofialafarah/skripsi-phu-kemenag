...
// Cek apakah ada ID pelimpahan di URL
if (!isset($_GET['id_pelimpahan']) || empty($_GET['id_pelimpahan'])) {
    echo "ID pelimpahan tidak ditemukan di URL.";
    exit();
}

$id_pelimpahan = intval($_GET['id_pelimpahan']);

// Ambil data pelimpahan berdasarkan ID dan ID jamaah
$query_select = "SELECT * FROM pelimpahan WHERE id_pelimpahan = ? AND id_jamaah = ?";
$stmt_select = $koneksi->prepare($query_select);
$stmt_select->bind_param("ii", $id_pelimpahan, $id_jamaah);
$stmt_select->execute();
$result = $stmt_select->get_result();

if ($result->num_rows === 0) {
    $_SESSION['error_message'] = "Data tidak ditemukan atau tidak memiliki akses.";
    header("Location: pelimpahan_jamaah.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['dokumen']) && $_FILES['dokumen']['error'] === UPLOAD_ERR_OK) {
        ...
        $target_dir = "uploads/pelimpahan/pengajuan/";
    ...
    $query_update = "UPDATE pelimpahan SET
        ...
        WHERE id_pelimpahan = ? AND id_jamaah = ?";
    ...
    if ($stmt_update->execute()) {
        $_SESSION['success_message'] = "Data pelimpahan berhasil diperbarui.";
        header("Location: pelimpahan_jamaah.php");
        exit();
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>Edit Pengajuan Pelimpahan</title>
            ...
            <main class="pPelimpahan-wrapper">
                <div class="pPelimpahan">
                    <div class="pPelimpahan-header" style="background-color: #1b5e20; color: white;">
                        <i class="fas fa-edit me-1"></i> Edit Pelimpahan Haji
                    </div>
                    <div class="pPelimpahan-body" style="color: #1b5e20;">
                        ...
                        <form method="POST" action="" enctype="multipart/form-data">
                            ...
                            <!-- ================================================================================================ -->
                            <div class="section-title">Keterangan Pelimpahan</div>
                            <hr>
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="kategori" id="kategori" class="select-daftar" required>
                                        <option value="" disabled>-- Pilih Jenis Kelamin --</option>
                                        <option value="Sakit Permanen" <?php echo (strtolower($data['kategori']) == 'Sakit Permanen') ? 'selected' : ''; ?>>Sakit Permanen</option>
                            ...
                            <div>
                                ...
                                <a href="pelimpahan_jamaah.php" class="btn btn-secondary">BATAL</a>
                            </div>
                        </form>
                    </div>
                </div>
